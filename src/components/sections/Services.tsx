...
const Services: React.FC = () => {
  ...
  return (
    ...
    >
      {/* Background system - optimized to prevent flickering */}
      <div className="absolute inset-0 bg-gray-50 dark:bg-gray-900" />
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-blue-50/30 to-transparent dark:via-blue-900/20 opacity-100" />

      {/* Minimal background decoration - simplified animations */}
      <div className="absolute inset-0 pointer-events-none opacity-40">
        <div className="absolute top-10 sm:top-20 right-10 sm:right-20 w-32 sm:w-48 lg:w-72 h-32 sm:h-48 lg:h-72 bg-blue-500/5 dark:bg-blue-500/10 rounded-full blur-3xl" />
        <div className="absolute bottom-10 sm:bottom-20 left-10 sm:left-20 w-40 sm:w-56 lg:w-80 h-40 sm:h-56 lg:h-80 bg-purple-500/5 dark:bg-purple-500/10 rounded-full blur-3xl" />
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-4 relative z-10">
        ...
        >
          ...
          {/* Value Propositions - optimized animations */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-5xl mx-auto">
            {[
              ...
              <motion.div
                ...
                className="flex flex-col items-center text-center space-y-3 sm:space-y-4 p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-white dark:bg-gray-800 backdrop-blur-sm border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 hover:shadow-lg transition-all duration-300 group"
              >
                <div
                  className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-gradient-to-r ${item.color} shadow-lg group-hover:shadow-xl transition-all duration-300`}
                >
        ...
        {/* Services Grid - optimized animations to prevent flickering */}
          ...
          {servicesData.map((service, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative bg-white dark:bg-gray-800 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-6 sm:p-8 border overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-xl ${
                  service.popular
                    ? "border-purple-500/30 ring-2 ring-purple-500/20"
                    : "border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600"
                }`}
              >
                {/* Popular Badge - simplified animation */}
                {service.popular && (
                ...
                {/* Icon - simplified hover animation */}
                <div className="relative z-10 mb-6 sm:mb-8">
                  <div
                    className={`inline-flex p-3 sm:p-5 rounded-2xl sm:rounded-3xl bg-gradient-to-r ${service.color} shadow-lg group-hover:shadow-xl transition-all duration-300`}
                  >
                ...
                {/* Features Preview - simplified animations */}
                <div className="relative z-10 space-y-2 sm:space-y-3 mb-6 sm:mb-8">
                  {service.features.slice(0, 4).map((feature, featureIndex) => (
                    ...
                    >
                      <HiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 sm:mr-3 mt-0.5 sm:mt-1 flex-shrink-0 transition-transform duration-200" />
                      <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 transition-colors duration-200 leading-relaxed">
                        {feature}
                      </span>
                    </div>
                  ))}
                  {service.features.length > 4 && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 font-semibold hover:text-blue-700 dark:hover:text-blue-300 transition-all duration-300 flex items-center px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/20"
                    >
                ...
                <div className="relative z-10 space-y-2 sm:space-y-3">
                  <motion.button
                    ...
                    className={`w-full py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base transition-all duration-300 flex items-center justify-center space-x-2 sm:space-x-3 shadow-lg hover:shadow-xl ${
                      service.popular
                  ...
                  >
                    ...
                    <HiArrowRight className="w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300" />
                  </motion.button>
            ...
            {addOns.map((addon, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`group bg-white dark:bg-gray-800 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-6 sm:p-8 border cursor-pointer transition-all duration-300 hover:shadow-xl ${
                    isSelected
                      ? "border-green-500/40 ring-2 ring-green-500/20"
                      : "border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600"
                  }`}
                >
                  {/* Rest of the addon content remains the same */}
                  <div className="flex items-start justify-between mb-4 sm:mb-6">
                    <div className="p-3 sm:p-4 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl sm:rounded-2xl shadow-lg transition-all duration-300">
                  ...
                  {/* Expandable Details - simplified animations */}
                  <motion.button
                    onClick={() =>
                      setExpandedAddOn(isExpanded ? null : addon.id)
                    }
                    whileHover={{ scale: 1.02 }}
                    className="w-full flex items-center justify-between text-xs sm:text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors duration-300 p-2 sm:p-3 rounded-lg sm:rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/20"
                  >
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-6 sm:px-8 lg:px-10 py-4 sm:py-5 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base lg:text-lg shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center space-x-2 sm:space-x-3"
              onClick={() => {
                document
            ...
            >
              ...
              <HiArrowRight className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 transition-transform duration-300" />
            </motion.button>

            <motion.button
              whileHover={{ scale: 1.05, y: -3 }}
              whileTap={{ scale: 0.95 }}
              className="px-6 sm:px-8 lg:px-10 py-4 sm:py-5 border-2 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base lg:text-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-300"
              onClick={() => {
                document
      ...
      {/* Service Details Modal - same as before */}
      <AnimatePresence>
              ...
              <div className="p-6 sm:p-8 lg:p-10">
                <div className="flex items-center justify-between mb-6 sm:mb-8">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    onClick={() => setShowServiceModal(false)}
                    className="p-2 sm:p-3 text-gray-400 hover:text-white hover:bg-white/10 rounded-full transition-all duration-300"
                  >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-10">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      {servicesData
                        ...
                        ?.features.map((feature, idx) => (
                          <motion.div
                            key={idx}
                            className="flex items-start p-2 sm:p-3 rounded-lg sm:rounded-xl hover:bg-white/5 transition-colors duration-200"
                  ...
                  <div>
                    ...
                    <motion.button
                      ...
                      className="w-full py-4 sm:py-5 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base lg:text-lg shadow-xl hover:shadow-2xl transition-all duration-300 relative overflow-hidden group"
                    >
